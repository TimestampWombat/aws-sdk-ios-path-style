...
static NSString *const AWSS3PreSignedURLBuilderSDKVersion = @"2.27.12";
...
- (AWSTask<NSURL *> *)getPreSignedURL:(AWSS3GetPreSignedURLRequest *)getPreSignedURLRequest {
    ...
    return [[[AWSTask taskWithResult:nil] continueWithBlock:^id(AWSTask *task) {
        ...
        if (!self.configuration.localTestingEnabled &&
          ...
          if(endpoint.portNumber && endpoint.portNumber > 0 ){
            bool httpWith80 = endpoint.useUnsafeURL && [endpoint.portNumber isEqualToNumber: @80];
            bool httpsWith443 = !endpoint.useUnsafeURL && [endpoint.portNumber isEqualToNumber: @443];
            if(!httpWith80 && !httpsWith443) {
              hostWithPort = [host stringByAppendingFormat:@":%@", endpoint.portNumber];
            }
          }
        }
        if(!hostWithPort) {
        ...
        [getPreSignedURLRequest setValue:hostWithPort forRequestHeader:@"host"];
